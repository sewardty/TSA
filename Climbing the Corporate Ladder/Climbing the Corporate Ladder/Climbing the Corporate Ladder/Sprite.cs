...
namespace Climbing_the_Corporate_Ladder
{
    class Sprite
    {
        ...
        {
            point.X = (int)((point.X) / 128f);
            point.Y = (int)(point.Y / 128);

            int map_width = World.map.GetLayer("Foreground").LayerWidth;
            int map_height = World.map.GetLayer("Foreground").LayerHeight;

            if (point.X < map_width && point.Y < map_height && point.X >= 0 && point.Y >= 0)
            {
                xTile.Tiles.Tile tile = World.map.GetLayer("Foreground").Tiles[(int)point.X, (int)point.Y];
                if (tile != null)
                {
        ...
        public virtual KeyValuePair<xTile.Tiles.Tile, Vector2> CollisionEdgeTest(Vector2 point1, Vector2 point2)
        {
            ...
            Vector2 cpoint1 = new Vector2((int)((point1.X) / 128f), (int)(point1.Y / 128));
            Vector2 cpoint2 = new Vector2((int)((point2.X) / 128f), (int)(point2.Y / 128));
            Vector2 cpoint3 = new Vector2((int)((point3.X) / 128f), (int)(point3.Y / 128));
